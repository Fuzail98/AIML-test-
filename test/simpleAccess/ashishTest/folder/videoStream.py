...
from folder.camera import Camera
...
class VideoStream(object):
    def __init__(self, sources, img_h, img_w, ROIs, POIs, models, device, fps=30):
        ...
        for i, (cam_id, src) in enumerate(sources.items()):
            ...
            camera = Camera(cam_id=cam_id, source=src, cap=cap, fps=fps, img_h=img_h,
                            img_w=img_w, ROIs=cam_ROIs, POIs=cam_POIs, models=cam_models, device=device)
        ...
        for cam_id, camera in self.cameras.items():
            camera.thread.start()
            print(
                '[bold yellow] VideoStream: Thread started for camera {} [/bold yellow]'.format(cam_id))
    ...
    def update(self, i, camera):
        ...
        while True:
            if camera.restart:
                ...
                print(
                    '[bold yellow] VideoStream: Thread restarted for camera {} [/bold yellow]'.format(camera.cam_id))
            ...
            if camera.cap.isOpened():
                ...
                if status:
                    ...
                    cv2.rectangle(frame, (0, 0), (130, 50),
                                  (255, 255, 255), -1)
                    cv2.rectangle(frame, (0, 0), (130, 50), (0, 0, 0), 2)
                    cv2.putText(frame, camera.cam_id, (10, 30),
                    ...
                    frame = np.ones(
                        (camera.img_h, camera.img_w, 3), dtype=np.uint8)*140
                    cv2.putText(frame, f'{camera.cam_id} stream ended', (10, 30),
